...
void odomHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
  ...
  geometry_msgs::Quaternion q_msg = odomMsg->pose.pose.orientation;
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("use_gazebo_odom", use_gazebo_odom);
  ...
  if (!use_gazebo_odom)
  {
    pubVehicleOdom = nh.advertise<nav_msgs::Odometry>("/state_estimation", 5);
  }
  else{
    odomSub = nh.subscribe<nav_msgs::Odometry>("/gazebo_odom", 5, odomHandler);
  }
  ...
  if (!use_gazebo_odom)
  {
    pubModelState = nh.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 5);
  }
